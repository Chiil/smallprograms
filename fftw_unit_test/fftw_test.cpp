        ...
        std::stringstream& ss_jplanb)
{
    ...
    fftw_plan iplanf = fftw_plan_many_r2r(rank, ni, jtot, fftini, ni, istride, idist,
                                fftouti, ni, istride, idist, kindf, FFTW_EXHAUSTIVE);
    fftw_plan iplanb = fftw_plan_many_r2r(rank, ni, jtot, fftini, ni, istride, idist,
                                fftouti, ni, istride, idist, kindb, FFTW_EXHAUSTIVE);
    fftw_plan jplanf = fftw_plan_many_r2r(rank, nj, itot, fftinj, nj, jstride, jdist,
                                fftoutj, nj, jstride, jdist, kindf, FFTW_EXHAUSTIVE);
    fftw_plan jplanb = fftw_plan_many_r2r(rank, nj, itot, fftinj, nj, jstride, jdist,
    ...
    ss_iplanf << fftw_sprint_plan(iplanf);
    ss_iplanb << fftw_sprint_plan(iplanb);
    ss_jplanf << fftw_sprint_plan(jplanf);
    ss_jplanb << fftw_sprint_plan(jplanb);
}
...
int main()
{
    ...
    fftini  = fftw_alloc_real(itot*jtot);
    fftouti = fftw_alloc_real(itot*jtot);
    fftinj  = fftw_alloc_real(jtot*itot);
    fftoutj = fftw_alloc_real(jtot*itot);
