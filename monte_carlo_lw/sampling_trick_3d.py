...
pos_sample = np.random.uniform(-dx/2, dx/2, size=(n_samples, 3))
...
def count_abs(pos):
    ...
    for i in range(n_samples):
        ...
        j = int((pos_sample[i, 1] + Ly/2) / dz)
        i = int((pos_sample[i, 2] + Lx/2) / dy)
        abs_sample[k, j, i] += 1
...
plt.subplot(132)
plt.pcolormesh(xh, zh, abs_ref[:, jtot//2, :])
plt.gca().add_patch(Rectangle((-dx/2, -dz/2), dx, dz, linestyle=':', fill=False))
plt.subplot(133)
plt.pcolormesh(yh, zh, abs_ref[:, :, itot//2])
