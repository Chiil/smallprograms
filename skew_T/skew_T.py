...
if __name__ == '__main__':
    ...
    T0 = np.arange(-100, 201, 20)

    dry_adiabat = T0[None, :] - 9.81/1005 * h[:, None]
    ...
    ax.semilogy(T, p, color='C3', label='T')
    ax.semilogy(Td, p, color='C2', label='Td')
    ax.legend(loc=1)

    ax.semilogy(dry_adiabat, p, color='#bbbbbb', linewidth=0.4)

    # ax.fill_betweenx(p, T, dry_adiabat, where=(dry_adiabat < T), color='C3', alpha=0.2, interpolate=True)
    # ax.fill_betweenx(p, T, dry_adiabat, where=(dry_adiabat > T), color='C0', alpha=0.2, interpolate=True)

    # An example of a slanted line at constant X
    # l = ax.axvline(0, color='k', linewidth=0.5)
