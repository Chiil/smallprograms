...
contains
    subroutine diff_f90(at, a, visc, dxidxi, dyidyi, dzidzi, itot, jtot, ktot)
        double precision, dimension(itot, jtot, ktot) :: at
        double precision, dimension(itot, jtot, ktot) :: a
        double precision :: visc, dxidxi, dyidyi, dzidzi
        integer :: itot, jtot, ktot

        do k = 2, ktot-1
            do j = 2, jtot-1
                do i = 2, itot-1
                    at(i,j,k) = a(i,j,k) + visc * ( &
                        + ( (a(i+1, j  , k  ) - a(i  , j  , k  )) &
