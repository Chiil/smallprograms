...
def psim(zeta):
    if(zeta <= 0):
        x = (1. - 16. * zeta) ** (0.25)
        psim = 3.14159265 / 2. - 2. * \
            np.arctan(x) + np.log((1. + x) ** 2. * (1. + x ** 2.) / 8.)
    else:
        psim = -2. / 3. * (zeta - 5. / 0.35) * \
...
def psih(zeta):
    if(zeta <= 0):
        ...
        psih = -2. / 3. * (zeta - 5. / 0.35) * np.exp(-0.35 * zeta) - \
...
print(
    'BD: z/L = {0}, db = {1}, B0 = {2}, ustar = {3}'.format(zL0_bd, db0, B0_bd, ustar_bd))
print(
    'W:  z/L = {0}, db = {1}, B0 = {2}, ustar = {3}'.format(zL0_w, db0, B0_w, ustar_w))
