...
def calc_s(t):
    return np.exp(-alpha*t) * (c0 - beta*(2*alpha*np.exp(alpha*t)*np.sin(2*t)+alpha**2*np.exp(alpha*t)*np.cos(2*t)+(-alpha**2-4)*np.exp(alpha*t))/(2*alpha**3+8*alpha))

def calc_s_impl(t_start, t_end, s_start):
    dt = t_end - t_start
    return (s_start + dt*beta*np.sin(t_end)**2) / (1. + dt*alpha)
...
tt_step = 0.25
ss_step = 0.05
tt, ss = np.meshgrid(np.arange(t.min(), t.max()+1, tt_step), np.arange(-.5*s0, 1.5*s0+0.001, ss_step))
...
plt.plot(t[0], s[0], 'ro', linewidth=1.)
plt.plot(t, s, 'k:', linewidth=1.)
plt.quiver(tt, ss, dsdt_tt, dsdt_ss, scale=0.06,
